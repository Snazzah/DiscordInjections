class Plugin {
    ...
    _loadCss() {
        ...
        let cssDirPath = window._path.join(this.path, 'css');
        try{
            let cssDir = window._fs.lstatSync(cssDirPath);
        }catch(e){
            return;
        }
        if (window._fs.lstatSync(cssDirPath).isDirectory()) {
            let files = window._fs.readdirSync(cssDirPath);
            let css = '';
            for (const file of files) {
                if (window._path.extname(file) !== '.css') continue;
                css += `/* ${file} */\n\n`;
                let content = window._fs.readFileSync(window._path.join(cssDirPath, file), { encoding: 'UTF8' });
                css += content + '\n\n';
            }
