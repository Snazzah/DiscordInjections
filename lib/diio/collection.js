module.exports = class Collection {
  ...
  add(data, initialize = false) {
    if (data instanceof this.cls) {
      ...
      let set = this._data.find(
        r => (this._selector ? this._selector(r) === data.id : r.id === data.id)
      )
  ...
  get(identifier) {
    ...
    if (this._loadedData.has(id)) return this._loadedData.get(id)

    let set = this._data.find(
      r => (this._selector ? this._selector(r) === id : r.id === id)
    )
  ...
  delete(identifier) {
    if (!identifier) return null
    const id = identifier.id || identifier
    if (this._loadedData.has(id)) return this._loadedData.delete(id)

    let set = this._data.find(
      r => (this._selector ? this._selector(r) === id : r.id === id)
    )
    if(set) this._data.splice(this._data.findIndex(r => r.id === id), 1)
    return null
  }
  ...
  find(compare) {
    ...
    while ((row = this._data.shift())) {
      ...
      if (compare(data)) return data
    }
  }

  forEach(cb) {
    // abuse our find callback
    this.find(row => {
      cb(row)
      return false
    })
